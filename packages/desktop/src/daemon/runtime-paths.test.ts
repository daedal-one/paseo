...
describe("runtime-paths", () => {
  beforeEach(() => {
    ...
    setProcessRuntime({
      platform: "darwin",
      execPath: "/Applications/Daedal DSH.app/Contents/MacOS/Daedal DSH",
      resourcesPath: "/Applications/Daedal DSH.app/Contents/Resources",
    });
  });
  ...
  it("uses the macOS Helper executable for packaged daemon node launches", () => {
    expect(resolveNodeExecPath()).toBe(
      "/Applications/Daedal DSH.app/Contents/Frameworks/Daedal DSH Helper.app/Contents/MacOS/Daedal DSH Helper",
    );
  });
});
