...
Choose **View sessions**, then **Read conversation**. The direct preview reads DSH history, streams new output, sends text, and answers approvals and questions through the installed DSH Client. It shares the browser and iPhone Session owners. Rich content, earlier history, attachments and other controls remain in the current DSH interface. Native host discovery remains migration work; the default companion discovery path is separate.
