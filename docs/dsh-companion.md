...
Use **Register workspace** in the opt-in native Host directory to enter an existing directory on that Host. A rejected request retains its path and diagnostic; **Edit path** durably clears the reviewed attempt before correction. Closing the form preserves a dispatched attempt under **Review registration**. An unknown outcome offers **Check current registration**; a found workspace requires **Use current registration** before continuing. This selects the current registration without confirming the earlier request. Then choose that workspace in **New session → Existing workspace**. Only identities in the current shared Host list are selectable.

The native runtime has a separate durable registration owner. It admits the generated Workspace create and canonical lookup operations independently. The Host resolves paths. An atomic journal in a dedicated SQLite or IndexedDB database retains the exact requested path and local ownership identity before one dispatch, surviving Host replacement and cold restart. The local identity is not a Host request receipt. Version-1 operation records remain readable without rewrite; new journal writes use version 2. Older journal readers may refuse those records. This local record format does not provide Session data downgrade support. Storage errors fail closed; forgetting pairing retains these non-credential records.

A create reply confirms registration. With Workspace create semantic revision 2, a qualified pre-write rejection retains the Host diagnostic and permits an explicit reset before correcting the path. This says only that the request began no registration write; it does not say the path has no registration. Other dispatched errors remain unknown because the Host's invalid-path response can also cover storage failures. An older create revision disables registration while compatible lookup and basic Sessions remain available. An explicit lookup reports current registration, absence or failure without changing the earlier mutation outcome. A positive observation can be explicitly adopted, recorded separately from confirmed creation. Generation change clears observations; reconnect performs no lookup or mutation. Only a confirmed, rejected, explicitly adopted or locally not-dispatched attempt permits a durable reset after its outcome has been saved. The current shared Workspace list remains the authority for selection, including deletion or re-registration after an observation.
